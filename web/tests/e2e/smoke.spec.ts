...
test('public games page paginates and supports page-size changes', async ({ page }) => {
  ...
  await Promise.all(
    Array.from({ length: 21 }, async (_unused, index) => {
...
test('public games page filters by players and date range while preserving pagination state', async ({
  page,
}) => {
  const ada = await createE2ePlayer({ name: 'Ada' });
  const bea = await createE2ePlayer({ name: 'Bea' });
  ...
  await expect(page.getByRole('button', { name: /filters/i })).toHaveAttribute(
    'aria-expanded',
    'true',
  );
...
test('admin login, edit game, and logout work', async ({ page }) => {
  ...
  const updatedGame = page
    .locator('main')
    .getByText('Updated harbor game')
    .locator('..')
    .locator('..');
...
test('admin can add, update, and delete players while blocking in-use deletes', async ({
  page,
}) => {
...
test('updating rate thresholds changes the matching stats views independently', async ({
  page,
}) => {
  ...
  await createE2eGame({
    notes: 'Ada encore',
    players: [{ playerId: ada.id, score: 13, isWinner: true }],
  });

  await page.goto('/stats');
  const winRateSection = page.locator('section#win-rate');
  const podiumRateSection = page.locator('section#podium-rate');
  ...
  await expect(podiumRateSection).toContainText('Bea');
  ...
  await expect
    .poll(async () => {
      await page.goto('/stats');
      return {
        winRate: await page.locator('section#win-rate').textContent(),
        podiumRate: await page.locator('section#podium-rate').textContent(),
      };
    })
    .toMatchObject({
      winRate: expect.stringContaining('Min 2 games'),
      podiumRate: expect.stringContaining('Min 4 games'),
    });

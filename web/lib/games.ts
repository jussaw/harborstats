...
/**
 * Applies an edit to an existing game and replaces its participant rows in a
 * single transaction. The `UPDATE ... RETURNING` runs first so the existence
 * check and the mutation are atomic: when no row matches (a stale/deleted id),
 * we stop before touching `game_players` so a no-op edit can't orphan-insert
 * participants against a missing game. Returns `true` when a game actually
 * matched and was updated, `false` when no game had that id — so callers can
 * tell a real update from a stale one and avoid recording a success audit for a
...
export async function updateGame(
  ...
  return db.transaction(async (tx) => {
    const updated = await tx
      .update(games)
      .set({ playedAt: input.playedAt, notes: input.notes })

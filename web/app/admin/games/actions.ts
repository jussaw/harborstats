...
export async function updateGameAction(formData: FormData): Promise<GameActionResult> {
  ...
  try {
    ...
    // Only mutate and audit a real update: a malformed/nonpositive id, or a
    // stale edit where no row matched, must not insert participants or record a
    // false game.update success. Either way we land back on the (now up-to-date)
    // games list below, so there's no user-visible error state to surface.
    if (

...
function formatTotalGames(count: number) {
  return `${count} total game${count === 1 ? '' : 's'}`
}

function formatAddedGames(count: number) {
  return `Added ${count} game${count === 1 ? '' : 's'} in this bucket`
}

function formatCumulativeAriaLabel(bucket: CumulativeActivityBucket) {
  return `${bucket.label}: ${bucket.cumulativeGames} cumulative game${bucket.cumulativeGames === 1 ? '' : 's'}, ${bucket.gameCount} added`
}

function buildAreaLayout(data: CumulativeActivityBucket[]) {
  const width = 640
  const height = 240
  const plotLeft = 52
  const plotRight = 24
  ...
  const maxCumulativeGames = Math.max(...data.map((bucket) => bucket.cumulativeGames), 1)
  const points = data.map((bucket, index) => {
    const x =
      data.length === 1 ? width / 2 : plotLeft + (innerWidth * index) / (data.length - 1)
    const y =
      plotTop + innerHeight - (bucket.cumulativeGames / maxCumulativeGames) * innerHeight
...
function getYAxisTicks(maxCumulativeGames: number) {
  return [...new Set([0, Math.ceil(maxCumulativeGames / 2), maxCumulativeGames])].sort(
    (a, b) => a - b,
  )
}
...
export function CumulativeGamesAreaChart({
  playedAtIsos,
  defaultView,
  timeZone = undefined,
}: Props) {
  ...
  const series = useMemo(() => {
    ...
    return buildCumulativeGamesSeries({
      playedAtIsos,
  ...
  const {
    ...
    areaPath,
    linePoints,
  } = buildAreaLayout(data)
  const activePoint = activeIndex === null ? null : points[activeIndex] ?? null
  const hoverWidth = points.length === 1
    ? width - plotLeft - plotRight
    : Math.max((points[1].x - points[0].x) / 1.5, 28)
  const yAxisTicks = getYAxisTicks(maxCumulativeGames)
  const xAxisLabelIndices = getXAxisLabelIndices(points.length)

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <StatsCardDetailSlot size="compact" className="
          flex-1 text-sm text-(--cream)/55
        ">
          {activePoint ? (
            <div>
              <p className="text-(--cream)">{activePoint.label}</p>
              <p className="mt-1 text-(--gold) tabular-nums">
                {formatTotalGames(activePoint.cumulativeGames)}
              </p>
              <p className="mt-1 tabular-nums">{formatAddedGames(activePoint.gameCount)}</p>
            </div>
          ) : (
            <p>Hover over a data point to inspect cumulative progress.</p>
          )}
        </StatsCardDetailSlot>
        <ActivityViewToggle
          view={view}
      ...
      >
        <svg
          ...
          role="img"
          aria-label={`Cumulative games over time (${view})`}
          onMouseLeave={() => setActiveIndex(null)}
        >
          {yAxisTicks.map((tick) => {
            const y = plotBottom - (tick / maxCumulativeGames) * (plotBottom - plotTop)
          ...
          {points.map((point, index) => (
            <g key={`${point.bucketStart}-target`}>
              <rect
                ...
                role="button"
                tabIndex={0}
                aria-label={formatCumulativeAriaLabel(point)}
